...
func TestCodexWebsocketsExecuteStreamTranslatesClaudeRequestToCodex(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		created := []byte(`{"type":"response.created","response":{"id":"resp-1","model":"gpt-5-codex"}}`)
	...
	opts := cliproxyexecutor.Options{
		...
		Metadata: map[string]any{
			cliproxyexecutor.RequestedModelMetadataKey: "gateway/gpt-5-codex",
		},
	}
	...
	for chunk := range stream.Chunks {
		...
		for _, line := range strings.Split(string(chunk.Payload), "\n") {
			...
			if data.Get("type").String() == "message_start" && data.Get("message.model").String() == "gateway/gpt-5-codex" {
				foundMessageStartModel = true
			}
		}
	}
	if !foundMessageStartModel {
		t.Fatalf("expected stream message_start model to use requested model")
	}
...
func TestCodexWebsocketsExecuteTranslatesClaudeRequestToCodex(t *testing.T) {
	...
	if got := gjson.GetBytes(resp.Payload, "model").String(); got != "gateway/gpt-5-codex" {
		t.Fatalf("model = %q, want requested model; payload=%s", got, resp.Payload)
	}
